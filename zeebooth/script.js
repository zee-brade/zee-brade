...
async function startCamera(){
  ...
  try{
    state.stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:"user",width:{ideal:1280},height:{ideal:1600}},audio:false});
...
async function setupHands(){
  ...
  state.hands.setOptions({maxNumHands:1,modelComplexity:1,minDetectionConfidence:.55,minTrackingConfidence:.55});
  state.hands.onResults(onHandResults);
  const loop=async()=>{ if(!state.cameraRunning || !state.hands)return; try{ await state.hands.send({image:$("#camera-video")}); }catch(e){} requestAnimationFrame(loop); };
