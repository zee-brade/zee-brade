...
function navigate(route){
  ...
  updateNavPill();
  if(route === "camera") setTimeout(initCameraPage,450);
  if(route === "photos") renderHistory();
  if(route === "home") renderHomeHistory();
  window.scrollTo({top:0,behavior:"smooth"});
}
function updateNavPill(){
  const active=$(`.nav-item[data-route="${state.route}"]`) || $(`.nav-item[data-route="home"]`);
  const pill=$("#nav-pill"); if(!active||!pill)return;
  const nav=active.parentElement.getBoundingClientRect(), r=active.getBoundingClientRect();
  pill.style.width=`${r.width}px`; pill.style.translate=`${r.left-nav.left}px 0`;
}
...
async function startCamera(){
  ...
  if(!navigator.mediaDevices?.getUserMedia){ fallback.querySelector("span").textContent="Camera unavailable"; fallback.querySelector("p").textContent="Use a modern browser on HTTPS or localhost."; return; }
  try{
    state.stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:"user",width:{ideal:960},height:{ideal:1280}},audio:false});
    video.srcObject=state.stream; await video.play(); state.cameraRunning=true; fallback.style.display="none"; updateStatus("Camera ready"); await setupHands();
  }catch(err){
    fallback.style.display="grid"; fallback.querySelector("span").textContent="Camera permission needed"; fallback.querySelector("p").textContent="Allow camera access, then reload or try again."; updateStatus("Camera blocked");
  }
}
function stopCameraIfLeaving(route){ if(route!=="camera" && state.stream){state.stream.getTracks().forEach(t=>t.stop()); state.stream=null; state.cameraRunning=false; state.hands?.close?.(); state.hands=null;} }
...
async function setupHands(){
  ...
  state.hands.setOptions({maxNumHands:1,modelComplexity:0,minDetectionConfidence:.55,minTrackingConfidence:.55});
  state.hands.onResults(onHandResults);
  let frameCounter=0; const loop=async()=>{ if(!state.cameraRunning || !state.hands)return; frameCounter++; if(frameCounter % 3 === 0){ try{ await state.hands.send({image:$("#camera-video")}); }catch(e){} } requestAnimationFrame(loop); };
  requestAnimationFrame(loop);
}
function onHandResults(res){
  const lm=res.multiHandLandmarks?.[0]; if(!lm){setGesture(false);return;}
  const extended=[fingerExtended(lm,8,6,5),fingerExtended(lm,12,10,9),fingerExtended(lm,16,14,13),fingerExtended(lm,20,18,17)];
  const wrist=lm[0], thumb=distance(lm[4],wrist)>distance(lm[3],wrist)*1.1;
  const count=extended.filter(Boolean).length+(thumb?1:0);
  setGesture(count===2 && extended[0] && extended[1]);
}
function fingerExtended(lm,tip,pip,mcp){return distance(lm[tip],lm[0])>distance(lm[pip],lm[0])*1.12 && distance(lm[tip],lm[mcp])>distance(lm[pip],lm[mcp])*1.05}
function distance(a,b){return Math.hypot(a.x-b.x,a.y-b.y,a.z-b.z)}
function setGesture(on){state.gestureBlur=on; $("#camera-stage").classList.toggle("gesture-blur",on); $("#gesture-indicator").classList.toggle("show",on)}
...
async function openEditor(){
  navigate("editor"); state.frameIndex=0; state.finalBlob=null; state.finalDataUrl=null; await sleep(420); renderFrameChoices(); await renderFinal();
}
...
function renderFinal(){
  return new Promise(resolve=>{
    const canvas=$("#final-canvas"), {w,h}=finalDimensions(); canvas.width=w;canvas.height=h; const ctx=canvas.getContext("2d");
    $("#render-loading").classList.add("show");
    setTimeout(()=>{renderFrame(ctx,w,h,state.frameIndex,state.captures);$("#render-loading").classList.remove("show");canvas.toBlob(blob=>{state.finalBlob=blob;state.finalDataUrl=canvas.toDataURL("image/jpeg",.94);resolve()},"image/jpeg",.94)},80)
  })
}
...
async function downloadFinal(){await renderFinal(); if(state.finalBlob)downloadBlob(state.finalBlob,`photobooth-${Date.now()}.jpg`);saveFinalToHistory()}
...
function downloadDataUrl(data,name){const a=document.createElement("a");a.href=data;a.download=name;a.click()}
...
function wireCamera(){
  ...
  $("#retake-btn").onclick=()=>{state.captures=[];state.finalBlob=null;state.finalDataUrl=null;navigate("camera")};
  $("#download-final-btn").onclick=()=>downloadFinal();$("#share-final-btn").onclick=()=>{if(state.finalDataUrl)shareImage(state.finalDataUrl)};
}
