...
async function takePhoto(){
  ...
  // The rendered camera viewport is the single source of truth. We crop the
  // sensor image to exactly the same aspect ratio and centered crop the user saw.
...
function saveFinalToHistory(){
  ...
  const items=getHistory();items.unshift({id:randomId(),dataUrl:state.finalDataUrl,shots:state.shots,size:state.size,frame:FRAME_STYLES[state.frameIndex].name,createdAt:new Date().toISOString()});setHistory(items);
}
