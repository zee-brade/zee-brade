...
const FRAME_STYLES = [
  {name:"Classic Booth", slug:"classic-booth"},
  {name:"Rose Ticket", slug:"rose-ticket"},
  {name:"Film Strip", slug:"film-strip"},
  {name:"Polaroid Club", slug:"polaroid-club"},
  {name:"Editorial Tape", slug:"editorial-tape"},
  {name:"Soft Flash", slug:"soft-flash"},
  {name:"Signature Booth", slug:"signature-booth"},
];
const FRAME_LAYOUTS = {
  1: [
    "Classic Booth", "Rose Ticket", "Film Strip", "Polaroid Club", "Editorial Tape", "Soft Flash", "Signature Booth"
  ],
  2: [
    "Classic Booth Duo", "Rose Ticket Duo", "Film Strip Duo", "Polaroid Duo", "Editorial Duo", "Soft Flash Duo", "Signature Duo"
  ],
  3: [
    "Classic Booth Trio", "Rose Ticket Trio", "Film Strip Trio", "Polaroid Trio", "Editorial Trio", "Soft Flash Trio", "Signature Trio"
  ],
  4: [
    "Classic Booth Grid", "Rose Ticket Grid", "Film Strip Grid", "Polaroid Grid", "Editorial Grid", "Soft Flash Grid", "Signature Grid"
  ]
};
const TOTAL_FRAME_STYLES = 28;

const SIZES = [
  {label:"Auto", cls:"ratio-auto"},
  {label:"4:3", cls:"ratio-standard"},
...
function sizeClass(size){return SIZES.find(s=>s.label===size)?.cls||'ratio-auto'}
...
function isDesktopLayout(){
  ...
  return width>=780 || (desktopUA && width>=600);
}
function getAutoAspect(){
  return isDesktopLayout() ? 16/9 : 3/4;
}
function getActiveAspect(){return state.size === 'Auto' ? getAutoAspect() : parseAspect(state.size)}
function fitCameraStage(){
  const stage=$('#camera-stage');
  if(!stage)return;
  const {width:vw,height:vh}=getViewportSize();
  const desktop=isDesktopLayout();
  const ratio=getActiveAspect();
  const gutter=desktop ? 72 : 24;
  const maxWidth=desktop ? Math.min(vw-gutter,1040) : Math.min(vw-gutter,640);
  const verticalBudget=desktop ? Math.min(vh*.68,700) : Math.min(vh*.64,680);
  let camW=Math.floor(Math.min(maxWidth,verticalBudget*ratio));
  let camH=Math.floor(camW/ratio);
  if(camH>verticalBudget){camH=Math.floor(verticalBudget);camW=Math.floor(camH*ratio);}
  const minW=desktop ? 360 : 230;
  const minH=desktop ? 240 : 230;
  camW=Math.max(minW,camW);
  camH=Math.max(minH,camH);
  stage.classList.toggle('desktop-camera',desktop);
  stage.classList.toggle('mobile-camera',!desktop);
  stage.style.width=`${camW}px`;
  stage.style.height=`${camH}px`;
  stage.style.maxWidth=`calc(100vw - ${gutter}px)`;
  stage.style.maxHeight='calc(100vh - 170px)';
  stage.style.aspectRatio=`${ratio}`;
}
function applyCameraSize(){
  const stage=$('#camera-stage');
  if(!stage)return;
  stage.classList.remove(...SIZES.map(x=>x.cls));
  stage.classList.add(sizeClass(state.size));
  stage.style.setProperty('--camera-ratio',`${getActiveAspect()}`);
  stage.setAttribute('data-ratio',state.size);
  fitCameraStage();
}
function syncCameraForViewport(){applyCameraSize();}
async function startCamera(){
  ...
  try{
    state.stream=await navigator.mediaDevices.getUserMedia({
      video:{facingMode:{ideal:'user'},aspectRatio:{ideal:4/3},width:{ideal:960,max:1280},height:{ideal:720,max:960},frameRate:{ideal:24,max:30}},
      audio:false
    });
...
async function loadHandsDetector(){
  ...
  try{
    ...
    state.hands.setOptions({maxNumHands:1,modelComplexity:0,minDetectionConfidence:.4,minTrackingConfidence:.4});
...
function onHandResults(res){
  ...
  if(!lm){state.gestureHits=0;setGesture(false);return;}
  const index=extendedFinger(lm,8,6,5);
  const middle=extendedFinger(lm,12,10,9);
  const ring=extendedFinger(lm,16,14,13);
  const pinky=extendedFinger(lm,20,18,17);
  const wrist=lm[0];
  const spread=distance(lm[8],lm[12]) > distance(wrist,lm[12])*.16;
  const peace=index && middle && !ring && !pinky && spread;
  state.gestureHits = peace ? Math.min(3, state.gestureHits + 1) : Math.max(0, state.gestureHits - 1);
  setGesture(state.gestureHits >= 2);
}
function extendedFinger(lm,tip,pip,mcp){
  const tipP=lm[tip], pipP=lm[pip], mcpP=lm[mcp], wrist=lm[0];
  const farther=distance(tipP,wrist) > distance(pipP,wrist)*1.08;
  const straight=jointAngle(lm[tip],pipP,mcpP)>145;
  return farther && straight;
}
...
function setupShotCount(){
  $$("#shot-count-options button").forEach(b=>{
    b.onclick=()=>{
      state.shots=Number(b.dataset.shots);
      $$("#shot-count-options button").forEach(x=>x.classList.toggle("active",Number(x.dataset.shots)===state.shots));
      if(state.captures.length>state.shots) state.captures=state.captures.slice(0,state.shots);
      state.frameIndex=Math.min(state.frameIndex,FRAME_STYLES.length-1);
      updateCameraControls(); renderCaptureSlots(); updateCaptureProgress();
      soundTone(540,.06,"sine",.02);
    };
  });
  $$("#shot-count-options button").forEach(b=>b.classList.toggle("active",Number(b.dataset.shots)===state.shots));
}
...
function playShutterLiquid(){
  const el=$("#shutter-liquid");
  if(!el)return;
  el.classList.remove("play");
  void el.offsetWidth;
  el.classList.add("play");
  setTimeout(()=>el.classList.remove("play"),780);
  soundCapturePop();
}
function soundCapturePop(){
  soundTone(720,.07,"sine",.026);
  setTimeout(()=>soundTone(1040,.12,"sine",.018),55);
}
...
async function takePhoto(){
  ...
  try{
    ...
    const displayW=viewport?.clientWidth||1, displayH=viewport?.clientHeight||1;
    const aspect=displayW/displayH;
    let cw, ch, sx, sy;
    if(vw/vh > aspect){
      ch=vh; cw=Math.round(ch*aspect); sx=Math.round((vw-cw)/2); sy=0;
    }else{
      cw=vw; ch=Math.round(cw/aspect); sx=0; sy=Math.round((vh-ch)/2);
    }
    cw=Math.max(2,Math.min(vw,cw)); ch=Math.max(2,Math.min(vh,ch));
    sx=Math.max(0,Math.min(vw-cw,sx)); sy=Math.max(0,Math.min(vh-ch,sy));
    ...
    $("#flash-overlay").classList.remove("fire"); void $("#flash-overlay").offsetWidth; $("#flash-overlay").classList.add("fire");
    soundShutter(); renderCaptureSlots();
    if(state.captures.length>=state.shots){await sleep(420); openEditor();}
  }catch(err){
    console.error(err);
    showToast("Could not capture this photo");
  }finally{
    state.capturing=false;
  }
}
function parseAspect(size){if(size==="4:3")return 4/3;if(size==="1:1")return 1;if(size==="4:5")return 4/5;if(size==="9:16")return 9/16;if(size==="16:9")return 16/9;return getAutoAspect()}
...
function openDrawer(type){
  ...
  if(type==="size"){title.textContent="Camera size";make(SIZES.map(x=>({name:x.label,desc:x.label+" preview",cls:x.cls})),item=>{
    state.size=item.name;
    applyCameraSize();
    updateCameraControls();
    $$("#drawer-content .drawer-option").forEach(b=>b.classList.toggle("active", b.querySelector("b")?.textContent===state.size));
  })}
...
function setHistory(items){
  let next=[...items].slice(0,30);
  while(next.length){
    try{localStorage.setItem(storageKey(),JSON.stringify(next));return true;}catch(e){
      next=next.slice(0,-1);
    }
  }
  try{localStorage.removeItem(storageKey());}catch(e){}
  showToast("Storage penuh — frame lama dibersihkan");
  return false;
}
function saveFinalToHistory(){
  if(!state.finalDataUrl)return;
  const items=getHistory();
  const record={id:randomId(),dataUrl:state.finalDataUrl,shots:state.shots,size:state.size,frame:currentFrameName(),createdAt:new Date().toISOString()};
...
function printImage(dataUrl){
  if(!dataUrl){showToast("Preparing your final image…");return;}
  const w=window.open("", "photobooth-print", "noopener,noreferrer");
  if(!w){showToast("Allow pop-ups to print the frame");return;}
  w.document.write(`<!doctype html><html><head><title>PHOTObooth Print</title><style>@page{margin:12mm}html,body{margin:0;min-height:100%;display:grid;place-items:center;background:#fff}img{max-width:100%;max-height:92vh;object-fit:contain}</style></head><body><img src="${dataUrl.replace(/"/g,'&quot;')}" alt="PHOTObooth frame"><script>window.onload=()=>{setTimeout(()=>window.print(),80)};<\/script></body></html>`);
  w.document.close();
}
function wireCamera(){
  ...
  $("#download-final-btn").onclick=()=>saveFinal();$("#print-final-btn").onclick=()=>{if(state.finalDataUrl)printImage(state.finalDataUrl);else showToast("Preparing your final image…")};
}
function updateStatusIdle(){updateStatus("Ready")}

function boot(){makeAmbient();initNav();wireCamera();renderHomeHistory();updateStatusIdle();window.addEventListener("resize",()=>{updateNavPill(); syncCameraForViewport();});window.visualViewport?.addEventListener("resize",syncCameraForViewport);window.addEventListener("orientationchange",()=>setTimeout(syncCameraForViewport,120));document.addEventListener("visibilitychange",()=>{if(document.hidden&&state.stream)state.stream.getTracks().forEach(t=>t.enabled=false);else if(state.stream)state.stream.getTracks().forEach(t=>t.enabled=true)});}
boot();
