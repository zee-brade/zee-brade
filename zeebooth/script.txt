...
function getAutoAspect(){
  ...
  // Auto behaves like a real camera UI: portrait on a phone, wide on a desktop.
  return mobile ? 3/4 : 16/9;
}
...
function fitCameraStage(){
  ...
  const maxWidth=mobile ? Math.min(window.innerWidth-24,620) : Math.min(window.innerWidth-80,920);
  const maxHeight=Math.max(mobile ? 300 : 360, Math.min(window.innerHeight*(mobile ? .62 : .62), mobile ? 620 : 720));
  const width=Math.max(220, Math.round(Math.min(maxWidth,maxHeight*ratio)));
