...
function sizeClass(size){return SIZES.find(s=>s.label===size)?.cls||'ratio-standard'}
...
async function takePhoto(){
  ...
  const aspect=parseAspect(state.size);
  let cw, ch;
  if(vw/vh > aspect){
    ch=vh; cw=Math.round(ch*aspect);
  }else{
    cw=vw; ch=Math.round(cw/aspect);
  }
...
function parseAspect(size){if(size==="4:3")return 4/3;if(size==="1:1")return 1;if(size==="9:16")return 9/16;if(size==="16:9")return 16/9;return 4/5}
...
function openDrawer(type){
  ...
  if(type==="size"){title.textContent="Camera size";make(SIZES.map(x=>({name:x.label,desc:x.label+" preview",cls:x.cls})),item=>{state.size=item.name;applyCameraSize();updateCameraControls();})}
