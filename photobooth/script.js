...
const S={view:'home',stream:null,facing:'user',aspect:'4:5',shots:1,countdown:3,gesture:true,sound:true,mirror:true,grid:true,filter:'clean',captures:[],busy:false,hands:null,handLoop:false,lastGesture:'none',sameGesture:0,pending:null,theme:'noir',layout:0,finish:'editorial',fit:'cover',posX:50,posY:50};
...
function read(){try{return JSON.parse(localStorage.getItem(KEY)||'{"photos":[],"history":[]}')}catch{return{photos:[],history:[]}}}
...
function updateHud(){const ready=!!S.stream; $('hudTimer').textContent=S.countdown?String(S.countdown).padStart(2,'0')+'s':'READY';$('hudMode').textContent='MANUAL';$('hudLens').textContent=S.mirror?'MIRROR':'1×';$('readyChip').textContent=ready?'AKTIF':'BELUM AKTIF';$('readyChip').style.color=ready?'#8fe2b1':'#8f99a3'}
...
function capture(){const v=$('video');if(!v.videoWidth)return null;const c=crop(v),w=ratio(S.aspect)>=1.25?1600:1400,h=Math.round(w/ratio(S.aspect)),o=document.createElement('canvas');o.width=w;o.height=h;const x=o.getContext('2d');x.save();if(S.mirror){x.translate(w,0);x.scale(-1,1)}const filters={clean:'none',soft:'contrast(1.02) saturate(.96) brightness(1.02)',mono:'grayscale(1) contrast(1.06)',warm:'sepia(.2) saturate(1.16) contrast(1.02)'};x.filter=filters[S.filter]||'none';x.drawImage(v,c.sx,c.sy,c.sw,c.sh,0,0,w,h);x.restore();return{dataUrl:o.toDataURL('image/jpeg',.95),aspect:S.aspect,filter:S.filter}}
...
async function shoot(){if(S.busy||S.captures.length>=S.shots)return;S.busy=true;$('hint').textContent='Pertahankan pose…';await countdown();const p=capture();if(p){S.captures.push(p);renderStrip();flash();sound(240,.16);await sleep(120)}S.busy=false;if(S.captures.length>=S.shots){$('hint').textContent='Sesi selesai. Lihat hasilnya atau foto ulang.';toast('Foto berhasil diambil.')}else{$('hint').textContent=`Foto ${S.captures.length} dari ${S.shots}. Siapkan pose berikutnya.`}updateActions()}
...
function reset(){S.captures=[];renderStrip();$('hint').textContent=S.gesture?'Sensor gesture siap. Shutter tetap manual.':'Shutter manual aktif.'}

function classify(l){const d=(a,b)=>Math.hypot(a.x-b.x,a.y-b.y),open=(tip,pip)=>d(l[tip],l[0])>d(l[pip],l[0])*1.08;const i=open(8,6),m=open(12,10),r=open(16,14),p=open(20,18);if(i&&m&&!r&&!p)return'peace';if(i&&m&&r&&p)return'open';const pinch=d(l[4],l[8])<.13,heart=d(l[4],l[20])<.2&&d(l[4],l[8])<.24;if(pinch&&heart)return'heart';return'none'}
function handsResults(res){if(!S.gesture||!res.multiHandLandmarks?.length){S.lastGesture='none';S.sameGesture=0;return}$('sensorStatus').textContent='Tangan terdeteksi';const g=classify(res.multiHandLandmarks[0]);if(g==='none'){S.lastGesture='none';S.sameGesture=0;return}if(g===S.lastGesture)S.sameGesture++;else{S.lastGesture=g;S.sameGesture=1}$('sensorStatus').textContent=`Gesture ${g.toUpperCase()} terdeteksi`;if(S.sameGesture===7){S.sameGesture=0;$('gestureLabel').classList.remove('pulse');void $('gestureLabel').offsetWidth;$('gestureLabel').classList.add('pulse')}}
function startHands(){if(S.handLoop||!window.Hands||!S.stream)return;S.hands=new Hands({locateFile:f=>`https://cdn.jsdelivr.net/npm/@mediapipe/hands/${f}`});S.hands.setOptions({maxNumHands:1,modelComplexity:1,minDetectionConfidence:.62,minTrackingConfidence:.62});S.hands.onResults(handsResults);S.handLoop=true;let last=0;(async function loop(ts=0){if(!S.stream?.active){S.handLoop=false;return}if(ts-last>85){last=ts;try{if($('video').videoWidth)await S.hands.send({image:$('video')})}catch{}}requestAnimationFrame(loop)})()}
...
function prepCamera(){setAspect(S.aspect);renderStrip();setGestureLabel();updateActions();setVideoFilter();if(S.stream?.active)$('permission').classList.add('off')}
...
function renderEditor(){if(!S.pending){try{S.pending=JSON.parse(sessionStorage.getItem('zeebooth.pending')||'null')}catch{}}if(!S.pending?.photos?.length){go('camera');return}const arr=activeLayouts();S.layout=Math.min(S.layout,arr.length-1);$('editorText').textContent=`${S.pending.photos.length} foto · ${S.pending.aspect} · pilihan frame terkurasi`;renderThemes();renderLayouts();renderFrame()}
...
async function save(){try{const data=await composite(),st=read(),stamp=now(),t=currentLayout(),photo={id:'p-'+Date.now(),dataUrl:data,createdAt:stamp,shots:S.pending.photos.length,aspect:S.pending.aspect,theme:S.theme,layout:t.name};st.photos.unshift(photo);st.photos=st.photos.slice(0,40);st.history.unshift({id:'h-'+Date.now(),createdAt:stamp,shots:photo.shots,aspect:photo.aspect,theme:S.theme,layoutName:t.name,preview:data});st.history=st.history.slice(0,40);if(!write(st)){toast('Penyimpanan penuh. Hapus beberapa foto terlebih dahulu.');return}sessionStorage.removeItem('zeebooth.pending');S.pending=null;renderPhotos();renderHistory();renderRecent();go('photos');toast('Frame berhasil disimpan.')}catch(e){console.error(e);toast('Gagal menyimpan hasil. Coba layout lain.')}}
function renderPhotos(){const st=read(),w=$('photosList'),e=$('photosEmpty');w.innerHTML='';e.style.display=st.photos.length?'none':'block';st.photos.forEach(p=>{const d=document.createElement('article');d.className='photo-card';d.innerHTML=`<img src="${p.dataUrl}" alt="saved frame"><div class="photo-body"><div><b>${p.layout||'Frame jadi'}</b><span>${fmt(p.createdAt)} · ${p.theme||'studio'}</span></div><div class="actions2"><button class="mini-btn" data-d="${p.id}">Unduh</button><button class="mini-btn" data-x="${p.id}">Hapus</button></div></div>`;w.appendChild(d)});w.querySelectorAll('[data-d]').forEach(b=>b.onclick=()=>{const p=st.photos.find(x=>x.id===b.dataset.d);if(p){const a=document.createElement('a');a.href=p.dataUrl;a.download='zeebooth-'+Date.now()+'.png';a.click()}});w.querySelectorAll('[data-x]').forEach(b=>b.onclick=()=>{const s=read();s.photos=s.photos.filter(x=>x.id!==b.dataset.x);write(s);renderPhotos();toast('Foto dihapus.')})}
...
function bind(){
 bindNav();readUploaded();$('enable').onclick=enableCamera;$('shutter').onclick=shoot;$('reset').onclick=reset;$('finish').onclick=()=>{if(S.captures.length!==S.shots){toast(`Ambil semua ${S.shots} foto terlebih dahulu.`);return}S.pending={photos:S.captures.slice(),aspect:S.aspect,createdAt:now()};sessionStorage.setItem('zeebooth.pending',JSON.stringify(S.pending));S.layout=0;go('editor')};
 ...
 document.querySelectorAll('.filter').forEach(b=>b.onclick=()=>{S.filter=b.dataset.filter;document.querySelectorAll('.filter').forEach(x=>x.classList.toggle('active',x===b));setVideoFilter()});$('gesture').onclick=()=>{S.gesture=!S.gesture;$('gesture').classList.toggle('on',S.gesture);setGestureLabel();$('sensorStatus').textContent=S.gesture?'Sensor tangan siap':'Mode gesture dimatikan'};
...
document.addEventListener('DOMContentLoaded',()=>{bind();try{S.pending=JSON.parse(sessionStorage.getItem('zeebooth.pending')||'null')}catch{}renderPhotos();renderHistory();renderRecent();setAspect(S.aspect);setGestureLabel();updateHud();initMotion();initParallax();});
