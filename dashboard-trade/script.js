...
function handleAuthLogin(event) {
    ...
    const email = authLoginForm.querySelector('input[name="email"]')?.value.trim();
    const password = authLoginForm.querySelector('input[name="password"]')?.value.trim();

    if (!email || !password) return;

    if (!state.user) {
        state.user = deriveLoginUser(email);
        saveUser();
    }

    openMainApp();
    showToast(state.language === "en" ? "Login successful. Welcome back." : "Login berhasil. Selamat datang kembali.");
}

function handleAuthSignup(event) {
    event.preventDefault();
    const name = authSignupName?.value.trim();
    const email = authSignupForm.querySelector('input[name="email"]')?.value.trim();
    const password = authSignupForm.querySelector('input[name="password"]')?.value.trim();

    if (!name || !email || !password) return;

    // Simpan identitas signup tanpa menandai setup selesai.
    localStorage.setItem("trazeel_auth_signup_v1", JSON.stringify({
        name,
        email,
        createdAt: new Date().toISOString()
    }));
    ...
    showToast(state.language === "en" ? "Account created. Complete your account setup." : "Akun dibuat. Lengkapi account setup lo.");
}
